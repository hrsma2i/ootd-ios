...
//  OOTD
//
//  Created by Hiroshi Matsui on 2024/10/19.
//
    ...
    func placeSubviews(in bounds: CGRect, proposal: ProposedViewSize, subviews: Subviews, cache: inout Void) {
        ...
        for row in result.rows {
            ...
            for index in row.range {
                ...
                // 元の実装だと、子ViewのTextが長すぎると省略されずに親Viewの横幅を超えてしまっていたので、 proposal には .unspecified ではなく親の proposal を渡す
    ...
    struct FlowResult {
        ...
        init(in maxPossibleWidth: Double, subviews: Subviews, alignment: Alignment, spacing: CGFloat?) {
            ...
            func finalizeRow(index: Int, idealSize: CGSize) {
                ...
                // 元の実装だと、子ViewのTextが長すぎると省略されずに親Viewの横幅を超えてしまっていたので、 bounds.width が maxPossiebleWidth を超えないようにする
                bounds.width = min(max(bounds.width, rowWidth), maxPossibleWidth)
